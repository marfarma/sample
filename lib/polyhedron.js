...
function PLH(deps) {
  ...
  this.indexes = {};
  this.indexes.id = new deps.Index({ fieldName: '__PLH__.id', unique: true });
  this.indexes.type_id = new deps.Index({ fieldName: '__PLH__.type_id', unique: false });
...
function Datastore(config) {
  ...
  for (var j in config.server) {
    if (config.server.hasOwnProperty(j)) {
      if (typeof config.server[j] === 'function') {
        this[j] = config.server[j].bind(this);
      } else {
        this[j] = config.server[j]; 
      }
    }
  }
  
  return this.create(this.database)
...
Datastore.prototype.create = function (name) {
  this.database = name;

  return this.openDb(this.database)
         .then((function (result) {
            this.db = result; 
            return this.db;
         }).bind(this));
